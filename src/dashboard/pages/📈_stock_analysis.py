...
def load_data():
    try:
        ...
        st.error(
            "Master table not found. Please run the data pipeline first (`scripts/run_stock_check.py`)."
        )
        st.stop()
...
with col1:
    ...
    if not df_normalized.empty:
        for ticker in tickers:
            base_price = df_normalized[df_normalized["identifier"] == ticker][
                "Close"
            ].iloc[0]
            df_normalized.loc[
    ...
    with st.container(border=True):
        ...
        if not df_normalized.empty:
            performance = (
                df_normalized.groupby("identifier")["Normalized Price"]
                .last()
    ...
    with st.container(border=True):
        ...
        for ticker in tickers:
            ...
            fig.add_trace(
                go.Scatter(
                    x=ticker_data["Date"],
                    y=ticker_data["Normalized Price"],
                    name=ticker,
                )
            )
    ...
    if not df_filtered.empty:
        ...
        for i, ticker in enumerate(tickers):
            with chart_cols[i % num_cols]:
                ...
                ticker_data["overbought_shifted"] = ticker_data["overbought"].shift(
                    1, fill_value=False
                )
                overbought_starts = ticker_data[
                    ticker_data["overbought"] & ~ticker_data["overbought_shifted"]
                ]["Date"]
                overbought_ends = ticker_data[
                    ~ticker_data["overbought"] & ticker_data["overbought_shifted"]
                ]["Date"]

                if len(overbought_starts) > len(overbought_ends):
                    overbought_ends = pd.concat(
                        [overbought_ends, pd.Series([ticker_data["Date"].iloc[-1]])],
                        ignore_index=True,
                    )
                ...
                ticker_data["oversold_shifted"] = ticker_data["oversold"].shift(
                    1, fill_value=False
                )
                oversold_starts = ticker_data[
                    ticker_data["oversold"] & ~ticker_data["oversold_shifted"]
                ]["Date"]
                oversold_ends = ticker_data[
                    ~ticker_data["oversold"] & ticker_data["oversold_shifted"]
                ]["Date"]

                if len(oversold_starts) > len(oversold_ends):
                    oversold_ends = pd.concat(
                        [oversold_ends, pd.Series([ticker_data["Date"].iloc[-1]])],
                        ignore_index=True,
                    )
