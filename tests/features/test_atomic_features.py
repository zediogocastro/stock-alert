...
def sample_data():
    return pl.DataFrame(
        {
            "date": pl.date_range(
                start=date(2026, 1, 1), end=date(2026, 1, 5), interval="1d", eager=True
            ),
...
def test_volatility(sample_data):
    ...
    df_stable = pl.DataFrame({"date": [1, 2, 3], "price": [10.0, 10.0, 10.0]})
...
def test_rsi_overbought():
    ...
    df_up = pl.DataFrame({"date": [1, 2, 3, 4], "price": [10, 20, 30, 40]})
